...
async def login_para_obtener_token(
    ...
    # Dependencias para endpoints protegidos (requieren autenticacion)

#dependencia para obtener el usuario actual a partir del token JWT incluido en la solicitud
async def get_current_user(
    ...
    try:
        payload = jwt.decode(token, SECRET_KAY, algorithms=[ALGORITHM])
...
# buscar el usuario en la base de datos 
   ...
   # Dependencia para verificar roles de usuario en endpoints protegidos 
class RoleChecker:
    def __init__(self, allowed_roles: list[str]):
        #recibe una lista de roles permitidos para acceder a un endpoint
        self.allowed_roles = allowed_roles

        def __call__(self, current_user: Annotated[Usuario, Depends(get_current_user)]) -> Usuario:

            if current_user.role not in self.allowed_roles:
                raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="No tenes los permisos necesarios para realizar esta accion")
            return current_user
